...
def lane_detection(cv_bgr, x_meter, y_meter, cols, rows, fontFace, fontScale, fontThickness):
    ...
    # パネル用画像を小さくする
    cv_bgr_ipm_white = cv2.resize(cv_bgr_ipm_white, (int(cols/3), int(rows/3)))
    ...
    if is_pass:
        ...
        color = (255,255,255)
        end_x, end_y = draw_text(cv_bgr_ipm_white,display_str,color,start_x=x_left,start_y=y_top,fontFace=fontFace, fontScale=fontScale, fontThickness=fontThickness)
